...
// Client is the RabbitMQ infrastructure driver for auth-service. It is
// consumer-only: it subscribes to company.events to keep the local
// user_tenant_memberships copy eventually consistent.
type Client struct {
	mu      sync.RWMutex
	amqpURL string
...
func (c *Client) connect() error {
	...
	for i := 0; i < 10; i++ {
		...
		if err == nil {
			log.Println("Auth Service RabbitMQ Driver: Connected successfully")
			break
		}
		log.Printf("Auth Service RabbitMQ connection attempt %d/10 failed: %v. Retrying in 2s...", i+1, err)
...
func (c *Client) watchConnection() {
	for {
		...
		log.Printf("Auth Service RabbitMQ Driver: Connection dropped (reason: %v). Reconnecting...", closeErr)
		...
		for {
			...
			if err := c.connect(); err == nil {
				...
				log.Println("Auth Service RabbitMQ Driver: Reconnection established successfully!")
				break
			}
			time.Sleep(2 * time.Second)
		}
...
// PublishEventWithConfirm is retained for symmetry but auth-service is a
// consumer-only service today; it should NOT publish company events.
