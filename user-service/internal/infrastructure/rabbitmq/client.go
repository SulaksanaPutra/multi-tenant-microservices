...
func (c *Client) connect() error {
	...
	if err := ch.Confirm(false); err != nil {
		log.Printf("User Service RabbitMQ Driver Warning: Failed to enable Publisher Confirms: %v", err)
	}
...
func (c *Client) watchConnection() {
	for {
		...
		log.Printf("User Service RabbitMQ Driver: Connection dropped (reason: %v). Reconnecting...", closeErr)
		...
		for {
			...
			if err := c.connect(); err == nil {
				...
				log.Println("User Service RabbitMQ Driver: Reconnection established successfully!")
				break
			}
			time.Sleep(2 * time.Second)
		}
