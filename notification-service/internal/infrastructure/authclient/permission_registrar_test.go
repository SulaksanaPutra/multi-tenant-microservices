...
func TestPermissionRegistrar_Register_SuccessAndIdempotency(t *testing.T) {
	...
	err := registrar.Register(ctx, "notification-service", perms)
	if err != nil {
	...
	if capturedPayload.Service != "notification-service" || len(capturedPayload.Permissions) != 1 {
...
func TestPermissionRegistrar_Register_ServerError(t *testing.T) {
	...
	err := registrar.Register(ctx, "notification-service", []PermissionItem{{Name: "notifications:read"}})
	if err == nil {
