...
func TestRequireJWT(t *testing.T) {
	...
	t.Run("valid JWT injects context claims", func(t *testing.T) {
		...
		w := httptest.NewRecorder()
		req, _ := http.NewRequest("GET", "/test", nil)
		token := signTestToken(t, privateKey, "ten_123", "usr_456", []string{"tenants:read"})
		...
		if capturedTenantID != "ten_123" || capturedUserID != "usr_456" {
...
func TestRequirePermission(t *testing.T) {
	...
	t.Run("missing permissions claim returns 403", func(t *testing.T) {
		r := gin.New()
		r.Use(RequirePermission("tenants:read"))
		r.GET("/test", func(c *gin.Context) { c.Status(http.StatusOK) })
	...
	t.Run("matching permission in claims returns 200", func(t *testing.T) {
		...
		r.Use(func(c *gin.Context) {
			c.Set(ContextKeyPermissions, []string{"tenants:read", "tenants:update"})
			c.Next()
		})
		r.Use(RequirePermission("tenants:read"))
		r.GET("/test", func(c *gin.Context) { c.Status(http.StatusOK) })
