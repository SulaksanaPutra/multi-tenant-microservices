...
func (m *mockDebtService) CreatePayableDebt(ctx context.Context, input service.CreatePayableDebtInput) (*service.PayableDebtOutput, error) {
	if m.createDebtFn != nil {
		return m.createDebtFn(ctx, input)
	}
	return &service.PayableDebtOutput{ID: "debt_1"}, nil
}
...
func TestOrderCreatedConsumer_HandleDelivery(t *testing.T) {
	...
	t.Run("success claims inbox, creates payable debt record and acks", func(t *testing.T) {
	...
	t.Run("duplicate inbox event skips debt creation and acks", func(t *testing.T) {
		mockAck := &mockAcknowledger{}
		inboxService := &mockInboxService{
			claimEventFn: func(txCtx context.Context, input service.ClaimInboxInput) (bool, error) {
				return true, nil
		...
		if created {
			t.Error("expected debt creation to be skipped for duplicate")
		}
	})
	...
	t.Run("debt recording failure nacks with requeue", func(t *testing.T) {
		mockAck := &mockAcknowledger{}
		debtService := &mockDebtService{
			createDebtFn: func(ctx context.Context, input service.CreatePayableDebtInput) (*service.PayableDebtOutput, error) {
