...
func TestPermissionRegistrar_Register_SuccessAndIdempotency(t *testing.T) {
	...
	perms := []PermissionItem{
		{Name: "tenants:read", Description: "Read tenant profiles"},
		{Name: "tenants:write", Description: "Update tenant profiles"},
	}

	err := registrar.Register(ctx, "tenant-service", perms)
	if err != nil {
	...
	if capturedPayload.Service != "tenant-service" || len(capturedPayload.Permissions) != 2 {
...
func TestPermissionRegistrar_Register_ServerError(t *testing.T) {
	...
	err := registrar.Register(ctx, "tenant-service", []PermissionItem{{Name: "tenants:read"}})
	if err == nil {
