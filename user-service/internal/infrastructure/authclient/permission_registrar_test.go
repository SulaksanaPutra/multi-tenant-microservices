...
func TestPermissionRegistrar_Register(t *testing.T) {
	...
	err := r.Register(context.Background(), "user-service", []PermissionItem{
	...
	if payload.Service != "user-service" {
		t.Errorf("expected service name 'user-service', got %q", payload.Service)
	}
	if len(payload.Permissions) != 1 || payload.Permissions[0].Name != "users:read" {
	...
	if err := r.Register(context.Background(), "user-service", nil); err != nil {
...
func TestPermissionRegistrar_doRegister_Non200(t *testing.T) {
	...
	err := r.doRegister(context.Background(), "user-service", []PermissionItem{{Name: "x"}})
	if err == nil {
...
func TestPermissionRegistrar_doRegister_ContextCanceled(t *testing.T) {
	...
	err := r.doRegister(ctx, "user-service", nil)
