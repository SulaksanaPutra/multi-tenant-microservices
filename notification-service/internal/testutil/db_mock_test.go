...
func TestMockDBExecutor(t *testing.T) {
	...
	t.Run("ExecContext fallback and custom fn", func(t *testing.T) {
		mock := &MockDBExecutor{}
		res, err := mock.ExecContext(ctx, "INSERT INTO notifications DEFAULT VALUES")
		if res != nil || err != nil {
	...
	t.Run("QueryContext fallback and custom fn", func(t *testing.T) {
		mock := &MockDBExecutor{}
		rows, err := mock.QueryContext(ctx, "SELECT * FROM notifications")
		if rows != nil || err != nil {
		...
		_, err = mock.QueryContext(ctx, "SELECT * FROM notifications")
	...
	t.Run("QueryRowContext fallback returns dummy row", func(t *testing.T) {
		mock := &MockDBExecutor{}
		row := mock.QueryRowContext(ctx, "SELECT * FROM notifications WHERE id = $1", 1)
		if row == nil {
	...
	t.Run("MockResult getters", func(t *testing.T) {
		...
		if id != 99 || err != nil {
			t.Errorf("expected LastInsertId 99/nil, got %d/%v", id, err)
		}

		affected, err := res.RowsAffected()
		if affected != 2 || err != nil {
			t.Errorf("expected RowsAffected 2/nil, got %d/%v", affected, err)
		}
	})
}
